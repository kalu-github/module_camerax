...
class CameraFragment : BaseFragment<LibCameraxFragmentCameraBinding>(R.layout.lib_camerax_fragment_camera) {
    companion object {
        private const val TAG = "moudle_camerax"
    ...
    override val binding: LibCameraxFragmentCameraBinding by lazy { LibCameraxFragmentCameraBinding.inflate(layoutInflater) }
    ...
    private var flashMode by Delegates.observable(FLASH_MODE_OFF) { _, _, new ->
        binding.btnFlash.setImageResource(
            when (new) {
                FLASH_MODE_ON -> R.drawable.lib_camerax_ic_flash_on
                FLASH_MODE_AUTO -> R.drawable.lib_camerax_ic_flash_auto
                else -> R.drawable.lib_camerax_ic_flash_off
            }
        )
    }
        ...
        binding.llTimerOptions.circularClose(binding.btnTimer) {
            ...
            binding.btnTimer.setImageResource(
                when (timer) {
                    ...
                    CameraTimer.OFF -> R.drawable.lib_camerax_ic_timer_off
                }
            )
        }
    ...
    private fun closeFlashAndSelect(@FlashMode flash: Int) =
        binding.llFlashOptions.circularClose(binding.btnFlash) {
            ...
            binding.btnFlash.setImageResource(
                when (flash) {
                    FLASH_MODE_ON -> R.drawable.lib_camerax_ic_flash_on
                    FLASH_MODE_OFF -> R.drawable.lib_camerax_ic_flash_off
                    else -> R.drawable.lib_camerax_ic_flash_auto
                }
            )
